...
#
# HA-tuned wiring of the reusable modules:
#   * NAT gateway per AZ
...
module "vpc" {
  ...
  single_nat_gateway = false # prod: one NAT per AZ for egress HA
...
module "eks" {
  ...
  # prod: full control-plane audit trail
...
###############################################################################
# Data tier — Aurora PostgreSQL (multi-AZ)
###############################################################################
...
module "rds" {
  ...
  instance_count  = var.rds_instance_count # 2 = writer + reader, multi-AZ
  master_username = "dbadmin"

  allowed_security_group_ids = [module.eks.cluster_primary_security_group_id]

  deletion_protection   = true  # prod: guard against accidental drop
